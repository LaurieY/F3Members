...
/************Get Total paid for this selection  ************/
 ...
 $SQL = "SELECT id,surname ,forename,membnum ,phone,mobile,email,membtype,location,paidthisyear,amtpaidthisyear,feewhere,datejoined,1 FROM members  ".$where_to_use." ORDER BY $sidx $sord ". $extrasort. " LIMIT $start , $limit"; 
...
function markpaid() { 
   ...
   $fytotals = $members->gettotals();
	 $arr = array('membnum' => $xnum, 'paidthisyear' => $xpaid, 'amtpaidthisyear' => $xpay,'fytotals'=>$fytotals);
	 	 $arrencoded= json_encode($arr);
	 $admin_logger->write('in editmember after jsonencode '.$arrencoded);
   echo $arrencoded;
